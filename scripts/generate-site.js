...
function renderCalendarPage(calendarCollections) {
  ...
  const script = [
    ...
    "    const links = [`<a class=\"text-link\" href=\"${escapeAttribute(buildGoogleCalendarUrl(occurrence))}\" target=\"_blank\" rel=\"noreferrer\">Add to Google Calendar</a>`];",
    "    if (occurrence.url) links.push(`<a class=\"text-link\" href=\"${escapeAttribute(occurrence.url)}\" target=\"_blank\" rel=\"noreferrer\">Event link</a>`);",
    ...
    "      const note = occurrence.inferenceNote ? `<p class=\"calendar-agenda-note\">${escapeHtml(occurrence.inferenceNote)}</p>` : '';",
    "      const links = renderOccurrenceLinks(occurrence);",
    "      const summaryMarkup = summary ? `<p class=\"calendar-agenda-summary\">${escapeHtml(summary)}</p>` : '';",
    "      const location = `${escapeHtml(occurrence.country)}${occurrence.city ? `, ${escapeHtml(occurrence.city)}` : ''}`;",
    "      return `<article class=\"calendar-agenda-item\"><p class=\"calendar-agenda-date\">${escapeHtml(getOccurrenceDateLabel(occurrence))}</p><h3>${escapeHtml(occurrence.name)}</h3><p class=\"community-description\">${location}</p>${summaryMarkup}${note}${links}</article>`;",
    "    }).join('');",
    "  };",
    ...
    "      const links = renderOccurrenceLinks(occurrence);",
    "      const location = `${escapeHtml(occurrence.country)}${occurrence.city ? `, ${escapeHtml(occurrence.city)}` : ''}`;",
    "      return `<article class=\"calendar-day-entry\"><p class=\"calendar-agenda-date\">${escapeHtml(getOccurrenceDateLabel(occurrence))}</p><h4>${escapeHtml(occurrence.name)}</h4><p class=\"community-description\">${location}</p>${note}${links}</article>`;",
    "    }).join('');",
    "  };",
