...
function buildCalendarLinkSet(relativeIcsPath, label) {
  ...
  return [
    ...
    { label: "iCalendar", url: subscriptionUrl },
    {
...
function renderCalendarPage(calendarCollections) {
  ...
  const script = [
    ...
    "      return `<article class=\"calendar-agenda-item\"><p class=\"calendar-agenda-date\">${getOccurrenceDateLabel(occurrence)}</p><h3>${occurrence.name}</h3><p class=\"community-description\">${occurrence.country}${occurrence.city ? `, ${occurrence.city}` : ''}</p>${summaryMarkup}${note}${link}</article>`;",
    "    }).join('');",
    "  };",
    ...
    "      const desktopEvents = dayItems.slice(0, 2).map((occurrence) => { const fullName = escapeAttribute(occurrence.name); return `<button class=\"calendar-event-pill\" type=\"button\" data-calendar-date=\"${iso}\" data-full-name=\"${fullName}\" title=\"${fullName}\" aria-label=\"${fullName}\"><span class=\"calendar-event-pill-label\">${occurrence.name}</span></button>`; }).join('') || '<span class=\"calendar-empty-slot\">No events</span>';",
...
function renderStyles() {
  return [
    ...
    ".calendar-event-pill { position: relative; display: flex; align-items: flex-start; width: 85%; max-width: 85%; min-width: 0; padding: 8px 10px; border-radius: 12px; background: #ddf3ee; border: 1px solid #8ecabf; text-decoration: none; color: var(--accent-strong); font-size: 11px; line-height: 1.25; text-align: left; }",
    ".calendar-event-pill-label { display: -webkit-box; min-width: 0; overflow: hidden; text-overflow: ellipsis; overflow-wrap: anywhere; word-break: break-word; -webkit-box-orient: vertical; -webkit-line-clamp: 2; line-clamp: 2; }",
