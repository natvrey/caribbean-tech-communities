...
function renderCalendarPage(calendarCollections) {
  ...
  const script = [
    ...
    "      cells.push(`<article class=\"calendar-day${isCurrentMonth ? '' : ' is-muted'}${isToday ? ' is-today' : ''}${isSelected ? ' is-selected' : ''}\">${isMobile() ? mobileButton : `<div class=\"calendar-day-head\"><span>${cursor.getUTCDate()}</span></div>`}${isMobile() ? mobileDots : `<div class=\"calendar-day-events\">${desktopEvents}</div>`}</article>`);",
    "      cursor = addDays(cursor, 1);",
    "    }",
